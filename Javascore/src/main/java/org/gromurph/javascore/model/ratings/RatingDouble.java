...
public abstract class RatingDouble extends Rating
{
    ...
    /**
     * should return the precision with which ratings are saved
     * used by toString to display rating, and PanelRatingDouble to round input
     */
    ...
    @Override public double getPrimaryValue()
    {
        return fValue;
    }
     ...
     * @param inV
     */
    @Override public void setPrimaryValue( double inV)
    {
    	double oldV = fValue;
        fValue = inV;
    ...
    @Override public String toString( boolean full)
    {
        ...
        if (fValue == Double.POSITIVE_INFINITY)
            sb.append( res.getString("RatingMaxValue"));
        else if (fValue == Double.NEGATIVE_INFINITY)
            sb.append( res.getString("RatingMinValue"));
        else
        {
    ...
    {
        for (Iterator iter = race.finishers(); iter.hasNext();)
        {
            Finish fin = (Finish) iter.next();
            if (fin.getEntry().getDivision().equals( div))
            {
                if ( fin.getFinishPosition().isValidFinish() &&
                     fin.getFinishTime() != SailTime.NOTIME &&
                     fin.getElapsedTime() < 0)
                {
                    warnings.add( MessageFormat.format(
                        res.getString( "WarningBoatFinishBeforeStart"),
    ...
    protected void validateValidFinishTime( Race race, Division div, WarningList warnings)
    {
        for (Iterator iter = race.finishers(); iter.hasNext();)
        {
            ...
            if (fin.getEntry().getDivision().equals( div))
            {
                ...
                {
                    warnings.add( MessageFormat.format(
                        res.getString( "WarningBoatNeedsFinishTime"),
                        new Object[] { fin.getEntry(), race } ));
                }
            }
        }
    ...
    /**
     * rounds a double containing time in milliseconds to the nearest whole
     * second, then returns that time in milliseconds
     * @Deprecated
     */
 ...
 *
 * Revision 1.12.2.1  2005/06/26 22:47:19  sandyg
 ...
 *
 * Revision 1.9  2003/03/28 03:07:44  sandyg
 ...
 *
 * Revision 1.7  2003/01/05 21:29:29  sandyg
 * fixed bad version/id string
 *
 * Revision 1.6  2003/01/04 17:29:10  sandyg
