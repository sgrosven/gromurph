...
public class Boat extends BaseObject {
	...
	@Override
	public void xmlWrite(PersistentNode node) //Document doc, String tag)
	{
	...
	public Rating getRating(String inSys) {
		return RatingManager.getCompatibleRating( inSys, fRatingList);
	}

	/**
	 * if rating of this system already exists, it is replaced otherwise the rating is added to the list
	 */
	...
	/*
	 * public void removeRating(Rating delRate) { fRatingList.remove( delRate); if (delRate != null)
	 * delRate.removePropertyChangeListener(this); firePropertyChange( REMOVERATING_PROPERTY, delRate, null); }
	 */
	...
	public void vetoableChange(PropertyChangeEvent de) throws PropertyVetoException {
		...
		/*
		 * if (de.getSource() == fRatingList) { String fName = de.getPropertyName(); Rating newRate = (Rating)
		 * de.getNewValue(); Rating oldRate = (Rating) de.getOldValue();
		 * 
		 * if (fName.equals("add")) { if (newRate != null) newRate.addPropertyChangeListener(this); firePropertyChange(
		 * ADDRATING_PROPERTY, null, newRate); } else if (fName.equals("remove")) { if (oldRate != null)
		 * oldRate.removePropertyChangeListener(this); firePropertyChange( REMOVERATING_PROPERTY, oldRate, null); } else
		 * if (fName.equals("replace")) { if (newRate != null) newRate.addPropertyChangeListener(this); if (oldRate !=
		 * null) oldRate.removePropertyChangeListener(this); firePropertyChange( CHANGERATING_PROPERTY, oldRate,
		 * newRate); } } else
		 */
	...
	/*
	 * public Rating getRating( String inName) { return (Rating) fRatingList.get( inName); }
	 * 
	 * public RatingList getRatingList() { return fRatingList; }
	 */
	...
	/*
	 * public Person getLastSkipper() { if (pSkippers == null) return null; if (pSkippers.size() == 0) return null;
	 * 
	 * return (Person) pSkippers.back(); }
	 * 
	 * public void setLastSkipper(Person s) { if (pSkippers.size() == 0) { pSkippers.add(s); firePropertyChange(
	 * "LastSkipper", null, s); } else if (! s.equals( pSkippers.back())) { pSkippers.remove( s); // kills any existing
	 * occurrences of s pSkippers.add( s); firePropertyChange( "LastSkipper", null, s); } // if lastskippers already is
	 * s nothing happens }
	 */
...
/**
 * $Log: Boat.java,v $ Revision 1.4 2006/01/15 21:10:37 sandyg resubmit at 5.1.02
 *
 ...
 *
 * Revision 1.10.2.1 2005/06/26 22:47:17 sandyg Xml overhaul to remove xerces dependence
 *
 ...
 *
 * Revision 1.8 2003/07/10 02:50:30 sandyg overrides a contradictory legacy one-design rating on xmlRead
 *
 * Revision 1.7 2003/04/27 21:03:26 sandyg lots of cleanup, unit testing for 4.1.1 almost complete
 *
 * Revision 1.6 2003/03/28 03:07:43 sandyg changed toxml and fromxml to xmlRead and xmlWrite
 *
 * Revision 1.5 2003/01/05 21:16:31 sandyg regression unit testing following rating overhaul from entry to boat
 *
