 ...
 **/
public class ActionReportSeriesStandingsMultiStage extends ActionReport {
	...
	private void reportForDivision(PrintWriter pw, AbstractDivision div, EntryList entries, boolean is1D,
		...
		for (int e = 0; e < rankingPoints.size(); e++) {
			SeriesPoints rp = rankingPoints.get(e);

			if (haveFinalsRaces) {
				Stage thisLastStage = getLastStageRaced(rp.getEntry());
			...
			posString = Long.toString(rp.getPosition());
			if (rp.isTied()) posString = posString + "T";
			...
			for (int r = startr; r < fRegatta.getNumRaces(); r++) {
				if (showThisRace[r]) {
					...
					RacePointsList rpl2 = rpl.findAll(rp.getEntry());
			...
			addTableCell(pw, SeriesPoints.format(rp), "right", SERIES_CELLCLASS);
