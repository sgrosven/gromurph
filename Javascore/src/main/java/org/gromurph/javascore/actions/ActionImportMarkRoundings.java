...
// program, JavaScore.
//
// Version: $Id: ActionImportMarkRoundings.java,v 1.6 2006/05/19 05:48:42 sandyg Exp $
//
...
import org.gromurph.javascore.gui.DialogImportMarkRoundings;
import org.gromurph.javascore.gui.DialogImportTable;


public class ActionImportMarkRoundings extends AbstractAction
{
    static ResourceBundle res = JavaScoreProperties.getResources();
    static ResourceBundle resUtil = org.gromurph.util.Util.getResources();

    public ActionImportMarkRoundings()
    {
        super( res.getString("MenuImportRoundings"));
        putValue( Action.MNEMONIC_KEY,
            new Integer( res.getString( "MenuImportRoundingsMnemonic").charAt(0)));
    }

    public void actionPerformed(ActionEvent parm1)
    {
        if (JavaScoreProperties.getRegatta().getNumRaces() == 0)
        {
            JOptionPane.showMessageDialog(
                null,
                res.getString( "MarksMessageCreateRaceFirst"),
                res.getString( "MarkMessageNoRacesFound"),
                JOptionPane.WARNING_MESSAGE);
                return;
        }

        DialogImportTable dialog = new DialogImportMarkRoundings( JavaScore.getInstance());
...
/**
 * $Log: ActionImportMarkRoundings.java,v $
